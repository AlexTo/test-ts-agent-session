// Resolves the deployed S3HttpAgent agent from runtime config and authenticates requests to it.
...
> => {
  ...
  const arn = config?.agentRuntimes?.['S3HttpAgent']?.arn;
  if (!arn) {
    throw new Error(
      `No deployed agent named 'S3HttpAgent' found in runtime configuration (application ${application}).`,
    );
  }
