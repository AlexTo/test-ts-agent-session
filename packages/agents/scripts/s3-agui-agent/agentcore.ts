// Resolves the deployed S3AguiAgent agent from runtime config and authenticates requests to it.
...
> => {
  ...
  const arn = config?.agentRuntimes?.['S3AguiAgent']?.arn;
  if (!arn) {
    throw new Error(
      `No deployed agent named 'S3AguiAgent' found in runtime configuration (application ${application}).`,
    );
  }
