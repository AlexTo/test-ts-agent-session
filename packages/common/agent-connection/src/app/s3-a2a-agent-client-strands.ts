...
/** Strands client for the S3A2aAgent A2A agent. */
export class S3A2aAgentClientStrands {
  static async create(): Promise<A2AAgent> {
    ...
    const agentRuntimeArn = config.agentRuntimes?.['S3A2aAgent']?.arn;
    if (!agentRuntimeArn) {
      throw new Error(
        `No connected agent runtime named 'S3A2aAgent' found in runtime configuration.`,
      );
    }
