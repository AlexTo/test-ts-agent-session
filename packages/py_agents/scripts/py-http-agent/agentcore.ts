// Resolves the deployed PyHttpAgent agent from runtime config and authenticates requests to it.
...
> => {
  ...
  const arn = config.agentRuntimes?.['PyHttpAgent']?.arn;
  if (!arn) {
    throw new Error(
      `No deployed agent named 'PyHttpAgent' found in runtime configuration (application ${application}).`,
    );
  }
